...
const WhoIWorkWith: React.FC = () => {
  ...
  return (
    <section className="py-24 bg-[#7B8B6F] text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {groups.map((group, idx) => (
            <div key={idx} className="bg-white/10 backdrop-blur-sm p-10 rounded-3xl border border-white/20 transition-all hover:bg-white/20 hover:-translate-y-2">
              <div className="w-14 h-14 bg-white text-[#7B8B6F] rounded-2xl flex items-center justify-center mx-auto mb-8">
                {group.icon}
              </div>
              <h4 className="text-2xl font-serif mb-4">{group.name}</h4>
              <p className="text-sm text-white/80 font-light leading-relaxed">{group.desc}</p>
            </div>
          ))}
        </div>
      </div>
