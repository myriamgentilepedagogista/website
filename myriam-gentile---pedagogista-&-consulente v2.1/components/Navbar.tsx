          ...
          <div className="grid grid-cols-3 items-center md:hidden w-full">
            <div className="flex justify-start">
              <button 
                ...
                className="p-2 -ml-2 text-[#4A3F35] hover:text-[#55634A] transition-colors"
              >
                <Menu size={24} />
              </button>
            </div>
            ...
            <div className="flex justify-end">
              <button 
                ...
                className="bg-[#55634A] text-[#FFFFFF] px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider hover:bg-[#46523D] transition-colors"
              >
                Contatti
              </button>
            </div>
            ...
            <div className="flex justify-center space-x-8">
              {navLinks.map((link) => (
                <a
                  ...
                  className="text-sm font-medium hover:text-[#55634A] transition-colors whitespace-nowrap"
                >
                  {link.name}
                </a>
              ))}
          ...
          <div className="flex-grow py-8 px-6 space-y-6">
            {navLinks.map((link) => (
              <a
                ...
                href={`/#${link.href}`}
                onClick={(e) => handleNavClick(e, link.href)}
                className="block text-2xl font-serif text-[#4A3F35] hover:text-[#96472D] transition-colors"
              >
                {link.name}
              </a>
            ))}
