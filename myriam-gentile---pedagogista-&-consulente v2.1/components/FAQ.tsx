      ...
      <div className="container mx-auto px-6">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-[#A05A3E] font-semibold tracking-widest text-xs uppercase mb-4 block">Chiarezza</span>
            ...
            {faqData.map((item, index) => (
              <div 
                ...
                className={`border rounded-3xl transition-all duration-300 ${
                  openIndex === index 
                    ? 'border-[#A05A3E]/30 bg-[#FDFBF7] shadow-sm' 
                    : 'border-[#EBE7E0] bg-white hover:border-[#A05A3E]/20'
                }`}
              >
                <button
                ...
                >
                  <span className={`text-lg md:text-xl font-serif transition-colors ${
                    openIndex === index ? 'text-[#A05A3E]' : 'text-[#4A3F35]'
                  }`}>
                    {item.question}
                  </span>
                  <div className={`shrink-0 ml-4 p-1 rounded-full transition-all ${
                    openIndex === index ? 'bg-[#A05A3E] text-white rotate-180' : 'bg-[#F9F6F1] text-[#6B6155]'
                  }`}>
                ...
                >
                  <div className="px-8 pb-8 text-[#6B5E51] font-light leading-relaxed">
                    <div className="w-8 h-px bg-[#A05A3E]/20 mb-4"></div>
                    {item.answer}
                  </div>
                </div>
              </div>
          ...
          <div className="mt-16 flex flex-col items-center gap-10">
            <Link 
              to="/faq" 
              className="inline-flex items-center gap-2 text-[#A05A3E] font-medium text-sm group hover:underline underline-offset-8 transition-all"
            >
              Consulta tutte le domande frequenti (FAQ) sulla pedagogia <ArrowRight className="w-4 h-4" />
            </Link>
            
            <div className="text-center">
              <p className="text-sm text-[#6B6155] italic mb-4">Hai altre domande o curiosità?</p>
              <button 
                onClick={onOpenContact}
                className="inline-flex items-center gap-2 text-[#4A3F35] font-medium border-b border-[#A05A3E] pb-1 hover:text-[#A05A3E] transition-colors"
              >
