    ...
    <div className="pt-32 pb-24 bg-[#FDFBF7] min-h-screen">
      ...
      <div className="container mx-auto px-6 max-w-4xl">
        <nav className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-[#A89E92] mb-8" aria-label="Breadcrumb">
          <Link to="/" className="hover:text-[#96472D] transition-colors">Home</Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#4A3F35] font-medium">FAQ</span>
        </nav>

        {/* Snippet Block */}
        <p className="mb-12 text-[#5A5046] font-light leading-relaxed text-sm bg-white/50 p-6 rounded-3xl border border-[#F3F0E9] text-center">
        ...
        <div className="mb-20 text-center">
          <HelpCircle className="w-16 h-16 text-[#96472D]/20 mx-auto mb-8" />
          ...
          {faqData.map((item, index) => (
            <div 
              ...
              className={`border rounded-[2.5rem] transition-all duration-300 ${
                openIndex === index ? 'border-[#96472D]/30 bg-white shadow-xl' : 'border-[#EBE7E0] bg-white hover:border-[#96472D]/20'
              }`}
            >
              <button
              ...
              >
                <span className={`text-xl font-serif transition-colors ${openIndex === index ? 'text-[#96472D]' : 'text-[#4A3F35]'}`}>
                  {item.question}
                </span>
                <div className={`shrink-0 ml-4 p-2 rounded-full transition-all ${openIndex === index ? 'bg-[#96472D] text-white rotate-180' : 'bg-[#F9F6F1] text-[#6B6155]'}`}>
                  {openIndex === index ? <Minus className="w-5 h-5" /> : <Plus className="w-5 h-5" />}
                </div>
              </button>
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openIndex === index ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="px-10 pb-10 text-[#6B5E51] font-light leading-relaxed text-lg">
                  <div className="w-8 h-px bg-[#96472D]/20 mb-4"></div>
                  {item.answer}
                </div>
              </div>
            </div>
        ...
        {/* CTA Section */}
        <div className="mt-24 p-12 bg-[#55634A] rounded-[3.5rem] text-white flex flex-col items-center text-center">
          <MessageCircle className="w-12 h-12 mb-6" />
          <h2 className="text-3xl font-serif mb-4">Hai altre curiosità o domande specifiche?</h2>
          <p className="text-white font-light mb-10 max-w-lg leading-relaxed">
            Ogni situazione educativa è unica. Se non hai trovato qui la risposta che cercavi, sarò felice di ascoltarti.
          </p>
          <button 
            onClick={onOpenContact}
            className="bg-white text-[#55634A] px-10 py-4 rounded-full font-medium hover:bg-[#F9F6F1] transition-all shadow-xl"
          >
        ...
        <div className="mt-24 pt-12 border-t border-[#F3F0E9]">
          <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#6B6155] mb-8">Approfondisci</h4>
          <div className="flex flex-wrap gap-x-12 gap-y-6">
            <Link to="/servizi" className="text-[#4A3F35] hover:text-[#96472D] font-serif text-xl transition-colors flex items-center gap-2 group">
              Servizi Pedagogici <ArrowUpRight className="w-4 h-4 opacity-50 group-hover:opacity-100 transition-all" />
            </Link>
            <Link to="/blog" className="text-[#4A3F35] hover:text-[#96472D] font-serif text-xl transition-colors flex items-center gap-2 group">
              Spazio Pedagogico <ArrowUpRight className="w-4 h-4 opacity-50 group-hover:opacity-100 transition-all" />
            </Link>
            <Link to="/chi-sono" className="text-[#4A3F35] hover:text-[#96472D] font-serif text-xl transition-colors flex items-center gap-2 group">
